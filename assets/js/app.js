$(function () {
  ...
  //* jQuery Swipe Event
  $(masonryCarouselZooFigure).swipe(
    function (direction) {
      if (direction == "left") {
        var active = $(".masonry-carousel-item.active");
        var next = $(active).next(".masonry-carousel-item");
  ...
  for (
    var _i = 0, recentWorksChildren_1 = recentWorksChildren;
    _i < recentWorksChildren_1.length;
    _i++
  ) {
    ...
    item.addEventListener("click", function (e) {
      ...
      var masonryIndex = masonryImagesArr.findIndex(function (item) {
        return e.currentTarget == item;
      });
  ...
  $(contactUsForm).on("submit", function (e) {
    e.preventDefault();
    var fields = $(e.currentTarget).find(
    ...
    $(fields).filter(function (index, item) {
      if ($(item).val().length == 0 || $(item).val() == "") {
        $(item)
          ...
          .css("display", "block");
      } else if ($(item).val().length == 1 && $(item).is("input[type=text]")) {
        $(item)
          .next(".contact-us-form-badge")
